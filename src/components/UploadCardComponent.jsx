...
function FileRow({ file, index, onRemove }) {
  // const color = EXT_COLORS[ext] || accent
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <MdFileUpload  style={{fontSize: '20'}}/>
      </div>
      <span style={{
    ...
    <div>
      ...
      <div
        ...
        style={{
          ...
          padding: files.length > 0 ? '22px 20px' : '40px 20px',
      ...
      >
        ...
        <div style={{ textAlign: 'center' }}>
          <p style={{ fontSize: 14, fontWeight: 600, color: '#374151', margin: '0 0 4px' }}>
            {isDragging ? 'Drop to upload' : files.length > 0 ? 'Drop more files or click to add' : 'Drag & drop files here'}
          </p>
          {files.length === 0 && (
      ...
      {files.length > 0 && (
        <div style={{ marginTop: 14 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, padding: '0 2px' }}>
            ...
            <button onClick={clearAll} style={{ fontSize: 12, color: '#9ca3af', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
              Clear all
            </button>
          </div>
